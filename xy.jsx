...
function main() {
	...
	prefs.layerPattern = prompt(' just "ok" click plz!! ', 'do not input');
...
function renameLayers(ref, prefs) {
   ...
   var len = ref.layers.length;
	...
	function rename() {
        ...
        var leftX = layer.bounds[0].value;
        var leftY = layer.bounds[1].value;
        var layername= ref.layers[i].name;
		...
		// rename layer
			...
			layer.name = layername +"     "+ "x:" + leftX + "    " + "y:" + leftY ;
			if (!vis) {
				layer.visible = false;
			}
			prefs.countFrom++;
